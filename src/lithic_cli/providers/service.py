...
def _init_provider_map() -> None:
    if _PROVIDER_MAP:
        return
    try:
        from lithic_cli.providers.anthropic_provider import AnthropicProvider

        _PROVIDER_MAP["anthropic"] = AnthropicProvider
    except ImportError:
        pass
    try:
        from lithic_cli.providers.openai_provider import OpenAIProvider

        _PROVIDER_MAP["openai"] = OpenAIProvider
    except ImportError:
        pass
    try:
        from lithic_cli.providers.ollama_provider import OllamaProvider

        _PROVIDER_MAP["ollama"] = OllamaProvider
    except ImportError:
        pass
    try:
        from lithic_cli.providers.openrouter_provider import OpenRouterProvider
